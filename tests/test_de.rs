...
{
    let deserialized: T = yaml_serde::from_str(yaml).unwrap();
    ...
    let deserialized: T = yaml_serde::from_value(value).unwrap();
    assert_eq!(*expected, deserialized);
    ...
    T: serde::de::Deserialize<'de> + PartialEq + Debug,
{
    ...
    yaml_serde::from_str::<yaml_serde::Value>(yaml).unwrap();
    yaml_serde::from_str::<serde::de::IgnoredAny>(yaml).unwrap();
}
...
fn test_i128_big() {
    ...
    let yaml = indoc! {"
        -9223372036854775809
    "};
    assert_eq!(expected, yaml_serde::from_str::<i128>(yaml).unwrap());

    let octal = indoc! {"
        -0o1000000000000000000001
    "};
    assert_eq!(expected, yaml_serde::from_str::<i128>(octal).unwrap());
}
...
fn test_u128_big() {
    ...
    "};
    assert_eq!(expected, yaml_serde::from_str::<u128>(yaml).unwrap());

    let octal = indoc! {"
        0o2000000000000000000000
    "};
    assert_eq!(expected, yaml_serde::from_str::<u128>(octal).unwrap());
}
...
fn test_de_mapping() {
    ...
    struct Data {
        pub substructure: yaml_serde::Mapping,
    }
    let yaml = indoc! {"
        substructure:
          a: 'foo'
    ...
    let mut expected = Data {
        substructure: yaml_serde::Mapping::new(),
    };
    expected.substructure.insert(
        yaml_serde::Value::String("a".to_owned()),
        yaml_serde::Value::String("foo".to_owned()),
    );
    expected.substructure.insert(
        yaml_serde::Value::String("b".to_owned()),
        yaml_serde::Value::String("bar".to_owned()),
    );
...
fn test_bomb() {
    ...
    assert_eq!(expected, yaml_serde::from_str::<Data>(yaml).unwrap());
}
...
fn test_numbers() {
    ...
    for &(yaml, expected) in &cases {
        let value = yaml_serde::from_str::<Value>(yaml).unwrap();
        match value {
...
fn test_nan() {
    ...
    assert!(yaml_serde::from_str::<f32>(".nan")
        .unwrap()
        .is_sign_positive());
    assert!(yaml_serde::from_str::<f64>(".nan")
        .unwrap()
        .is_sign_positive());
}
...
fn test_no_required_fields() {
    ...
    for document in ["", "# comment\n"] {
        ...
        let deserialized: NoRequiredFields = yaml_serde::from_str(document).unwrap();
        assert_eq!(expected, deserialized);

        let expected = Vec::<String>::new();
        let deserialized: Vec<String> = yaml_serde::from_str(document).unwrap();
        assert_eq!(expected, deserialized);

        let expected = BTreeMap::new();
        let deserialized: BTreeMap<char, usize> = yaml_serde::from_str(document).unwrap();
        assert_eq!(expected, deserialized);

        let expected = None;
        let deserialized: Option<String> = yaml_serde::from_str(document).unwrap();
        assert_eq!(expected, deserialized);

        let expected = Value::Null;
        let deserialized: Value = yaml_serde::from_str(document).unwrap();
        assert_eq!(expected, deserialized);
    }
}
...
fn test_empty_scalar() {
    ...
    let yaml = "thing:\n";
    let expected = Struct {
        thing: yaml_serde::Sequence::new(),
    };
    test_de(yaml, &expected);

    let expected = Struct {
        thing: yaml_serde::Mapping::new(),
    };
    test_de(yaml, &expected);
}

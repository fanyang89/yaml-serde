    ...
    I: Index,
{
    ...
    /// Index into a `yaml_serde::Value` using the syntax `value[0]` or
    /// `value["k"]`.
    ///
    ...
    /// let data: yaml_serde::Value = yaml_serde::from_str(r#"{ x: { y: [z, zz] } }"#)?;
    ///
    /// assert_eq!(data["x"]["y"], yaml_serde::from_str::<Value>(r#"["z", "zz"]"#).unwrap());
    /// assert_eq!(data["x"]["y"][0], yaml_serde::from_str::<Value>(r#""z""#).unwrap());
    ///
    /// assert_eq!(data["a"], yaml_serde::from_str::<Value>(r#"null"#).unwrap()); // returns null for undefined values
    /// assert_eq!(data["a"]["b"], yaml_serde::from_str::<Value>(r#"null"#).unwrap()); // does not panic
    /// # Ok(())
    /// # }
    /// ```
...
where
    I: Index,
{
    /// Write into a `yaml_serde::Value` using the syntax `value[0] = ...` or
    /// `value["k"] = ...`.
    ///
    ...
    /// ```
    /// # fn main() -> yaml_serde::Result<()> {
    /// let mut data: yaml_serde::Value = yaml_serde::from_str(r#"{x: 0}"#)?;
    ///
    /// // replace an existing key
    /// data["x"] = yaml_serde::from_str(r#"1"#)?;
    ///
    /// // insert a new key
    /// data["y"] = yaml_serde::from_str(r#"[false, false, false]"#)?;
    ///
    /// // replace a value in a sequence
    /// data["y"][0] = yaml_serde::from_str(r#"true"#)?;
    ///
    /// // inserted a deeply nested key
    /// data["a"]["b"]["c"]["d"] = yaml_serde::from_str(r#"true"#)?;
    ///
    /// println!("{:?}", data);
    /// # Ok(())
    /// # }
